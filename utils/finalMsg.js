const axios = require('axios');

const WATI_ACCESS_TOKEN = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..TDK1BKQwppc9P6MMLluWGOQrOisdZCtDGcCmoEpld6Q';
const phoneNumber = '919643782690'; // Update recipient number

/**
 * Sends thank you or follow-up question based on passed array.
 * @param {string[]} [followUps] - Optional array of follow-up questions.
 */
const sendThankYouOrFollowUp = async (followUps = []) => {
  let messageText = '';

  if (!followUps.length) {
    messageText = `Thank you for your lovely review! 🙌 We're so glad to have you with us & look forward to your next booking! 😊`;
  } else {
    messageText = `Thanks a ton for the review! 🙏 We had a quick read and it'd be super helpful if you could answer this too:\n\n🎤 "${followUps[0]}"\n\nA quick audio message (30–60 sec) would be perfect! 🎧`;
  }

  try {
    const response = await axios.post(
      `https://live-mt-server.wati.io/458328/api/v1/sendSessionMessage/${phoneNumber}?messageText=${encodeURIComponent(messageText)}`,
      {},
      {
        headers: {
          Authorization: `Bearer ${WATI_ACCESS_TOKEN}`,
          Accept: '*/*',
        },
      }
    );
    console.log('✅ Message sent:', response.data);
  } catch (error) {
    console.error('❌ Error:', error.response?.data || error.message);
  }
};

// === USAGE EXAMPLES ===

// Case 1: No follow-ups
sendThankYouOrFollowUp();

// Case 2: With follow-up
// sendThankYouOrFollowUp([
//   'What did you love the most about the experience?'
// ]);
